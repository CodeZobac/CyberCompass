...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Require authentication to view shared profiles
    if (!session?.user?.id) {
      return NextResponse.json(
        { error: 'Authentication required to view shared profiles' },
        { status: 401 }
      );
    }
    ...
    // Validate token and get the profile owner
    ...
    const profileUserId = shareLink.user_id;
    ...
    // Calculate analytics using the same function from the regular profile analytics
    ...
    return NextResponse.json({
      analytics,
      userInfo: {
        name: shareLink.users.name,
        // Don't expose email for privacy
        joinDate: shareLink.users.created_at,
      },
      shareInfo: {
        viewCount: shareLink.view_count + 1, // Include the current view
        expiresAt: shareLink.expires_at,
        createdAt: shareLink.created_at,
      },
    });
  } catch (error) {
    console.error('Error in shared profile API:', error);
    return NextResponse.json(
...
function calculateProfileAnalytics(
  ...
  // Calculate ethical development score
